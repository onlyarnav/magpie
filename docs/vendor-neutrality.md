...
**Overall vendor-neutrality score: 8/10 capability contracts (80%).** Generated by [`tools/vendor-neutrality-score`](../tools/vendor-neutrality-score/); re-run it to refresh this section.
...
| `contract:change-request` | ❌ | vendor-backed | GitHub | only 1 backend vendor (GitHub); needs 1 more |
...
**Per-skill assessment: 40/63 skills carry no vendor lock-in.** A skill is *capability-pure* when it names no backend at all, *portable* when every backend it names has an alternative (its contract is green), and *vendor-coupled* only when it reaches for a backend that is the sole implementation of a capability.

| Skill neutrality | Count |
|---|---|
| capability-pure (names no backend) | 9 |
| portable (named backends are swappable) | 31 |
| vendor-coupled (sole-backend dependency) | 23 |

Organization scope (declared, orthogonal to vendor): ASF = 14, agnostic = 49.

Vendor-coupled skills (the only lock-ins today):

- `audit-finding-fix` — `GitHub` → `contract:change-request`
- `committer-onboarding` — `GitHub` → `contract:change-request`
- `issue-fix-workflow` — `GitHub` → `contract:change-request`
- `mentoring-welcome` — `GitHub` → `contract:change-request`
- `pr-management-code-review` — `GitHub` → `contract:change-request`
- `pr-management-mentor` — `GitHub` → `contract:change-request`
- `pr-management-quick-merge` — `GitHub` → `contract:change-request`
- `pr-management-stats` — `GitHub` → `contract:change-request`
- `pr-management-triage` — `GitHub` → `contract:change-request`
- `release-announce-draft` — `GitHub` → `contract:change-request`
- `release-audit-report` — `GitHub` → `contract:change-request`
- `release-prepare` — `GitHub` → `contract:change-request`
- `release-vote-tally` — `GitHub` → `contract:change-request`
- `reviewer-routing` — `GitHub` → `contract:change-request`
- `security-issue-fix` — `GitHub` → `contract:change-request`
- `security-issue-import` — `Google` → `contract:mail-draft`
- `security-issue-import-from-pr` — `GitHub` → `contract:change-request`
- `security-issue-import-from-scan` — `GitHub` → `contract:change-request`
...
- `security-issue-triage` — `GitHub` → `contract:change-request`
- `setup-override-upstream` — `GitHub` → `contract:change-request`
<!-- END vendor-neutrality-score -->

### What the number means

80% is not "80% done." It reads as: **eight of ten capabilities already
work across more than one vendor, and the two that don't — the
pull-request review/merge gate (`change-request`) and outbound mail
drafting (`mail-draft`) — are each one adapter away.** The architecture
privileges no vendor on any of the ten axes; the two red cells are
missing *implementations* (a second forge that reviews/merges proposed
changes, e.g. GitLab or Gitea; and a second `mail-draft` backend),
tracked in the open — not a design that assumes GitHub or Gmail. The
change-request gap is the larger of the two: `contract:tracker` is green
because Jira also handles issues, but Jira has no pull-request model, so
every skill that drives `gh pr` (review, merge, triage) is coupled to
GitHub today. Ship one non-GitHub review/merge backend and all of those
skills become portable without a line of skill code changing — the seam
is the contract, not the workflow.
